...
### 1. Hook Extraction (reduced App.tsx by ~100 lines)
- `useTheme` → src/hooks/useTheme.ts
- `useWorkingHours` → src/hooks/useWorkingHours.ts  
- `useCalendarSettings` → src/hooks/useCalendarSettings.ts
- `useOfflineDetection` → src/hooks/useOfflineDetection.ts

### 2. Shared Component Extraction
- `PanelSpinner` → src/components/PanelSpinner.tsx (used by 10+ Suspense boundaries)
- `isAllDayTimeString()` → src/lib/utils.ts (shared utility)

### 3. Google Calendar Rate Limiting
- Added `withBackoff()` exponential backoff wrapper in src/lib/google.ts
- Retries 429 (rate limit) and 5xx errors: 3 attempts with 1s/2s/4s delays
- `GoogleAuthError` now carries `statusCode` field for robust retry logic
- Applied to all 5 API functions: fetchCalendarEvents, createCalendarEvent, 
  updateCalendarEvent, deleteCalendarEvent, fetchCalendarList

### 4. Notes on Skipped Items
- **Screen component extraction**: Attempted but abandoned — the massive props interfaces 
  (~30 props per screen) created more maintenance burden than they saved. App.tsx is an 
  orchestrator; its size is inherent to its role.
- **Lucide icon optimization**: 34 import sites across the project, but all heavy 
  components are already lazy-loaded so icon code is code-split. The 13 icons in App.tsx 
  are used in the critical render path.
- **E2E tests**: Require `scripts/capture-auth.mjs` to capture Google OAuth state first. 
  Cannot run headlessly without pre-captured credentials.
